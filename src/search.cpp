...
bool Search::Worker::iterative_deepening() {
           ...
           && !(limits.depth && mainThread && rootDepth >= limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            // Here `delta` is a radius, whereas other uses of the word mean a diameter
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!rootNode)
    {
        ...
        // when being mated. In this case, return a fail-high score.
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        // if the result is lower than ttValue minus a margin, then we will extend the
        // ttMove. If however the excluded search fails high then the ttMove is
        // non-singular and we may do some pruning or reduction instead.
        //
    ...
    // return a fail low score. We also update histories and stats.
    ...
    // Step 25. Return our final esimated value to the parent.
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    // Step 11. Return our final esimated value to the parent.
    assert(-VALUE_INFINITE < bestValue && bestValue < VALUE_INFINITE);
    return bestValue;
}
