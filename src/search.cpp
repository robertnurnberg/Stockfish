           ...
           && !(limits.depth && mainThread && rootDepth >= limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
                    ...
                    || rootMoves[pvIdx].score_is_exact_loss()))
            {
                ...
                // Otherwise we cap the score to the best possible, and mark the
                // score as a bound (also a valid excuse for the incomplete PV.)
                else
                {
...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    ss->followPV = rootNode
                || ((ss - 1)->followPV
                    && (static_cast<size_t>(ss->ply - 1) < rootMoves[pvIdx].previousPV.size()
                        && (ss - 1)->currentMove == rootMoves[pvIdx].previousPV[ss->ply - 1]));
